...
In order to estimate the thermal conductivity from EMD with the GK approach, one starts by computing the energy current $\mathbf{J}$ from a MD trajectory in the microcanonical ensemble,\footnote{\LEnote{***Comment on THERMOSTATS?***}} 
...
\begin{figure}[!htb]
    ...
    \LEnote{*** CAMBIA $f$ IN $\omega/2\pi$ ***}
\end{figure}

Fig.~\ref{fig:periodograms} displays the periodograms of the heat fluxes of four paradigmatic cases (liquid Ar, liquid H$_2$O, crystalline MgO, and amorphous SiO$_2$) obtained from a $100\un{ps}$ ($500\un{ps}$ for MgO) classical MD trajectory and averaged over the three Cartesian components, showing the extremely noisy behavior of the periodogram as an estimator of the spectrum. 
...
\begin{figure}[!tb]
    ...
    \caption{Cepstral coefficients of \LEnote{***...***} computed analyzing the low-frequency region of the periodogram (see Fig.~\ref{fig:periodograms}), defined in Eq.~\eqref{eq:sample-cepstrum}. }
    \label{fig:cepstrums}
\end{figure}
...
\LEnote{*****DA RIVEDERE L'UTILIZZO DI $f$ E $\omega$***********}\\
...
\begin{itemize}
    ...
    \item \textbf{crystalline fcc Mgo}: Buckingham-plus-Coulomb potential as in Ref. \citenum{MgO-FF} at a temperature $T\approx 1000\un{K}$ and density $\rho=3.61\un{g/cm^3}$, in a $4\times 4\times 4$ simple cubic conventional supercell with 512 atoms, and a time step $\Delta t=0.3\un{fs}$
...
\section{Outlook}
\LEnote{**Conclusioni cepstral methods, improvements, outlook... AIMD simulations...***}