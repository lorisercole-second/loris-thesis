...
In Section~\ref{sec:carbogno} \LEnote{**O FORSE IN APPENDICE**} we comment on some of the classical definitions of the energy flux, using the gauge invariance principle. 
...
\begin{figure}
    ...
    \LEnote{***RIMUOVERE UNITA' DI $\kappa$***}
    }
...
\begin{theorem}[Marcolongo-Schwartz] \label{th:aris-theorem}
...
\end{equation}
where $\mathcal{D}^X(\tau)=\int_0^\tau \mathbf{J}^X(t) dt$ is the displacement associated with the flux $\mathbf{J}^X$. It follows that:
\begin{equation}
...
\LEnote{*** Spiegazione dei metodi di scorrelazione / rinormalizzazione di Aris, per rimuovere i contributi delle self-energie. (non so se riuscir\`o / se ha senso parlare anche dei metodi multi-componente ***}
\LEnote{** Ma forse questa sezione va spostata nel cap.4 o 5 ?***}


\section{Comment on the classical definition of heat flux in solids}  \label{sec:carbogno}
\LEnote{*** dimostrazione semplice corrente ``leyla'' = corrente di calore, mentre quella ``di carbogno'' è sbagliata ***  SE RIESCO LA SPOSTEREI IN APPENDICE ***}
