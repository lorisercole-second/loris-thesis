...
In Section~\ref{sec:MolecularFluids}, we treat the specific case of molecular fluids and give a perspective on how the gauge invariance property may be exploited to define equivalent formulations for the heat currents. \LE{*** These ideas may reveal to be very handy in optimizing the statistical properties of the heat currents: a task especially useful in view of the \emph{ab initio} calculation of thermal conductivity, and that can be carried out using some techniques sketched in Section~\ref{sec:gauge-renormalization}.}
