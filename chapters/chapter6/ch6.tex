...
In order to study the convergence of lattice thermal conductivity with the size of the system and the quenching rate, we considered cells containing $72$, $144$, $288$, $432$, $576$, $864$, $1152$, $2304$, $5184$, and $10800$ atoms. For each cell size we analyzed the data obtained from $10$ independent replicas. 
...
We computed the thermal conductivity $\kappa$ of each of these systems, from a trajectory of $t_\mathrm{run}=1\un{ns}$, using \emph{cepstral analysis}. More technical details on this procedures are presented in Sec.~\ref{sec:cepstral-analysis}. 
...
For each $N_{at}$ and $\gamma$, we computed the weighted average of each replica's $\kappa$, depicted with a solid line, using the errors estimated from cepstral analysis as weights. 
We can infer that $\kappa$ is underestimated by the smallest systems considered, and it seems to converge to a stable value for $N_{at}\gtrsim 500$ atoms. 
This is likely due to two effects, both entering this analysis. 
One is the fact that small replicas cannot account for the whole ensemble of possible amorphous structures. In particular, the variety medium- and long-range structures need larger system to be correctly accounted for. However, as we mentioned in Sec.~\ref{sec:silica-size}, some authors found that averaging over a little set of small structures ($N_{at}=72$) should be enough to reproduce the range of glass-network structures found in larger cells \cite{VanGinhoven2005}. 
\LEnote{The VDOS is also weakly affected by the system size.}
Nevertheless, thermal conductivity needs a sufficient cell size to converge. We can probably attribute this behavior mostly to finite-size effects, that dictate an upper limit to the wavelength of the vibrational modes that can be accommodated into the simulation cell, and alter the the low and medium vibrational frequencies, which are largely attributed to collective modes. The Green-Kubo equation thus requires a larger volume to correctly account for all the contributions. We should however notice that this value is remarkably lower than the typical cell lengths required by NEMD calculations (\LEnote{NAT necessari in NEMD}).
...
Slower quenching rates (longer quenching times) definitely lead to higher thermal conductivities. 
...
A quenching rate $\gamma \lesssim 10^{13}\un{K/s}$ seem to be the requirement for almost all the simulated sizes. 

In Fig.~\ref{fig:results-class-kappa-vs-size} and \ref{fig:results-class-kappa-vs-quench} the error on the weighted average is indicated with a thin shaded area surrounding the solid line. Two other lines are reported. The dashed lines delimit the average error that we expect to obtain from a single trajectory sample of $t_{\mathrm{run}}=1\un{ns}$ (it is about the error on the weighted averaged times $\sqrt{10}$). 
We observe that in general the single calculations of $\kappa$ ($10$ replicas for each abscissa), represented by single circles, fall mostly inside this error bars, for the medium and large systems. For the smaller systems, the fluctuations of the single values become bigger, probably due to larger differences in their structures, in addition to the intrinsic large fluctuations due to the smaller size. This is also indicated by the standard deviation of $\kappa$ computed over the replicas, indicated with thin dotted lines, that becomes visibly larger than the expected error bars for the two or three smallest systems analyzed \LEnote{(even though this value, computed over only $10$ replicas may fluctuate a lot and should be taken as indicative)}.

\begin{figure}[!tb]
    \centering
    \subfigure[\label{fig:results-class-kappaerror-vs-size}]{\includegraphics[width=0.49\textwidth]{chapters/chapter6/figures/silica_NVT_kappa_error_NAT.pdf}}
    \hfill
    \subfigure[\label{fig:results-class-kappaerror-vs-quench}]{\includegraphics[width=0.49\textwidth]{chapters/chapter6/figures/silica_NVT_kappa_error_QTIME.pdf}}
    \caption{Thermal conductivity error}
    \label{fig:results-class-kappaerror}
\end{figure}

\LEnote{*RIGUARDA CON RELATIVE ERROR*}
The trend of the error, estimated for a $1\un{ns}$ trajectory, with $N_{\mathrm{at}}$ is shown in Fig.~\ref{fig:results-class-kappaerror-vs-size} for $4$ selected quenching rates. The estimated error decreases as $\sim 1/\sqrt{N_\mathrm{nat}}$, \LEnote{suggesting that it is influenced by the fluctuations of ... the energy?? in the NVE ensemble, that decrease with the same trend.}
The quenching rate, instead, does not change much the error on the estimate of $\kappa$, with a slight increase of error by lowering the quenching rate.



SIZE EFFECTS:  SMALL SAMPLE PROBLEMS -- SIZE EFFECTS OF GK

ERRORE CLASSICO DA 100ps
