...
The trend of the error, estimated for a $1\un{ns}$ trajectory, with $N_{\mathrm{at}}$ is shown in Fig.~\ref{fig:results-class-kappaerror-vs-size} for $4$ selected quenching rates. The estimated relative error decreases as $\sim 1/\sqrt{N_\mathrm{nat}}$, \LEnote{suggesting that it is influenced by the fluctuations of ... the energy?? in the NVE ensemble, that decrease with the same trend.}
The quenching rate, instead, does not appear to affect the error on the estimate of $\kappa$. 
